    ...
    public class ManageProductService : IManageProductService
    {
        ...
        public async Task<int> Update(ProductUpdateRequest request)
        {
            ...
            if (product == null || productTranslations == null)
                throw new EShopException($"Cannot find this product with " +
                    $"the id: {request.Id}");
            ...
            if (request.ThumbnailImage != null)
            {
                var thumbnailImage = await _context
                    .ProductImages
                ...
                if (thumbnailImage != null)
                {
                    ...
                    thumbnailImage.ImagePath = await 
                        this.SaveFile(request.ThumbnailImage);
        ...
        public async Task<int> Delete(int productId)
        {
            var product = await _context.Products
                .FindAsync(productId);
            if (product == null)
                throw new EShopException($"Cannot find this product:" +
                    $"{productId}");

            var images = _context.ProductImages.Where(i => 
            ...
            GetAllPaging(GetManageProductPagingRequest request)
        {
            ...
            if (!string.IsNullOrEmpty(request.Keyword))
                query = query.Where(x => 
                        x.pt.Name.Contains(request.Keyword));

            if(request.CategoryIds.Count > 0)
            {
                query = query.Where(p => request.CategoryIds
                        .Contains(p.pic.CategoryId));
            }
        ...
        #region Image
        public async Task<int> AddImage(int productId,
            ProductImageCreateRequest request)
        {
            ...
            {
                productImage.ImagePath = await this
                            .SaveFile(request.ImageFile);
        ...
        {
            var productIamge = await _context.ProductImages
                                                .FindAsync(imageId);
            if (productIamge == null)
                throw new EShopException($"Cannot find an image " +
                    $"with this id {imageId}");
            _context.ProductImages.Remove(productIamge);
            return await _context.SaveChangesAsync();
        }

        public async Task<int> UpdateImage(int imageId, 
        ...
        public async Task<List<ProductImageViewModel>> 
            GetListImages(int produtcId)
        {
            return await _context.ProductImages
                .Where(x => x.ProductId == produtcId)
        ...
        private async Task<string> SaveFile(IFormFile file)
        {
            ...
            await _storageService.SaveFileAsync(file.OpenReadStream(),
                                                fileName);
            return fileName;
        }
