...
namespace eShopSolution.BackendApi.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
            ...
            string languageId)
        {
            ...
            if (product == null)
                return BadRequest("Cannot find this product");
            return Ok(product);
        }
        ...
        [HttpPut]
        public async Task<IActionResult> Update([FromForm]
            ProductUpdateRequest request)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            var affectedResult = await _manageProductService
                                            .Update(request);
            if (affectedResult == 0)
                return BadRequest();
            return Ok();
